...
export default function TourDetails() {
  ...
  const { data: divisionData } = useGetDivisionQuery({ _id: data?.[0]?.division,fields:"name" },{skip:!data})
 
  const { data: tourTypeData } = useGetTourTypesQuery({ _id: data?.[0]?.tourType, fields: "name" }, { skip: !data })
